Shader "Custom/Flow map"
{
    Properties
    {
        ...
        _FlowSpeed ("speed", Range(0.1, 2)) = 0.5
    ...
    SubShader
    {
        Pass
        {
            ...
            float4 frag(VOUT IN) : SV_TARGET
            {
                ...
                float4 flow0 = tex2D(_FlowMap, uv_f0);
                float4 flow1 = tex2D(_FlowMap, uv_f1);
                ...
                float3 A = lerp(A_p0, A_p1, uv_p1.w);

                float3 S = lerp(float3(0.04f, 0.04f, 0.04f), A, _Metalic);
